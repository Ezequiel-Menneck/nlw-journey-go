...
func (q *Queries) CreateTrip(ctx context.Context, pool *pgxpool.Pool, params spec.CreateTripRequest) (uuid.UUID, error) {
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: failed to begin trx fro CreateTrip: %w", err)
	}
...
func (q *Queries) InviteParticipantToATrip(ctx context.Context, params InviteParticipantsToTripParams) error {
	participants := make([]InviteParticipantsToTripParams, 1)
	participants[0] = InviteParticipantsToTripParams{
		TripID: params.TripID,
		Email:  params.Email,
	}

	_, err := q.InviteParticipantsToTrip(ctx, participants)
	if err != nil {
		return fmt.Errorf("pgstore: failed to invite participants to trip: %w", err)
	}
